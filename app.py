...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://credit-desk-agf.onrender.com"],
    allow_methods=["*"],
    allow_headers=["*"],
)
...
BASE_URL = "https://agf.fiulive.finfactor.co.in/finsense/API/V2"

# Finsense credentials loaded server-side only
FINSENSE_USER_ID = os.getenv("AGF_FINSENSE_USER_ID", "")
FINSENSE_PASSWORD = os.getenv("AGF_FINSENSE_PASSWORD", "")
    ...
    try:
        ...
        try:
            row_id = insert_credit_analysis({
                ...
                'sanction_pct_min': round(result['sanction_percentage'][0], 2),
                'sanction_pct_max': round(result['sanction_percentage'][1], 2),
                'sanction_min': s_min,
                'sanction_max': s_max,
            })
        except Exception as e:
